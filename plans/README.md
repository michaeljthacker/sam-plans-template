...
- `plans/VISION.md` — the concept brief / seed for the Build (human-authored; actions read it but never modify it)
...
The human may also pre-declare size in `plans/VISION.md` (an explicit `size: phase-only` line, or natural-language signals like "patch" / "MVP" / "full prototype"); `Product.ProductVision` will respect it.
...
### 1. Idea & framing
- Human writes the concept brief in `plans/VISION.md` (one sentence to a few paragraphs) — the durable seed for the Build, left untouched for future reference
- `Product.ProductVision` → reads `plans/VISION.md` and generates root `README.md` + `plans/BUILD.md`
...
**Prunability gate.** Length alone does not trigger maintenance. Before routing to `PM.ThreadMaintenance`, `PM.StatusUpdate` runs a cursory check: given the current `config.json` and state, is there anything that *can* actually be pruned or promoted right now? Content protected by an `every_milestone` gate on the still-open milestone (its CodeReview, PhaseApproval, or DocumentationUpdate hasn't run yet) is not prunable, so a long-but-still-needed thread is left alone rather than churned through a no-op maintenance pass. `PM.ThreadMaintenance` also has a no-op guard for the same reason. Maintenance always runs at milestone closeout, where the whole milestone's thread becomes prunable.
...
3. Write your project idea in `plans/VISION.md` — anything from a one-sentence goal to a detailed concept brief. `Product.ProductVision` reads from `VISION.md`, so this is where your idea must live. It is the durable seed for the Build: actions read it but never modify it, and it is not pruned like `thread.md`. (Legacy setups that put the idea in `thread.md` still work — `Product.ProductVision` falls back to it.)
